...
//modified from SystemClock_Config()
void SystemClockConfig(uint32_t clk)
{
    ...
    //HAL_InitTick(TICK_INT_PRIORITY);					//KL commented out
    return;
}
...
//reset the mcu
void mcuInit(void) {
	//update systemcoreclock - default = 80Mhz
#if defined(CPU_CLK)
	SystemCoreClockConfig(CPU_CLK);				//set clocks and updates clocks
#endif
	SystemCoreClockUpdate();					//update the clock regardless
	...
	HAL_Init();
#else
	//reconfigure systick / coretimer - a down counter
...
inline void pinMode(PIN_TypeDef pin, uint8_t mode) {
	...
	switch (mode) {
		case OUTPUT: 			pinPullup(pin, 0); GIO_OUT(gpio, mask); break;
		case OUTPUT_OD: 		pinPullup(pin, 1); GIO_OUT(gpio, mask); break;
		case INPUT: 			pinPullup(pin, 0); pinPulldown(pin, 0); GIO_IN(gpio,  mask); break;
		case INPUT_PULLUP:		pinPullup(pin, 1); pinPulldown(pin, 0); GIO_IN(gpio,  mask); break;
		case INPUT_PULLDOWN:	pinPullup(pin, 0); pinPulldown(pin, 1); GIO_IN(gpio,  mask); break;
		default: 				pinPullup(pin, 0); pinPulldown(pin, 0); GIO_IN(gpio,  mask); break;
	}
}
...
inline void pinPullup(PIN_TypeDef pin, uint8_t en) {
	...
	if (en) IO_CLR(gpio->PULLUP_EN, mask);				//0->enable pull up
	else IO_SET(gpio->PULLUP_EN, mask);
}
...
inline void pinPulldown(PIN_TypeDef pin, uint8_t en) {
	...
	if (en) IO_SET(gpio->PULLDOWN_EN, mask);				//1->enable pull down
	else IO_CLR(gpio->PULLDOWN_EN, mask);
}
...
inline void pinAFIO_(PIN_TypeDef pin, uint8_t af) {
	...
	if (af & (1<<1)) IO_SET(gpio->AF_S1, mask); else IO_CLR(gpio->AF_S1, mask);	//1->enable af_s1
	if (af & (1<<0)) IO_SET(gpio->AF_S0, mask); else IO_CLR(gpio->AF_S0, mask);	//1->enable af_s0
	IO_SET(gpio->AF_SEL, mask);	//1->enable af
}
...
inline void pinTouch(PIN_TypeDef pin) {
	...
	//configure af_s1 and af_s0
	IO_CLR(gpio->AF_SEL, mask);				//1->enable af, 0->gpio
	IO_CLR(gpio->DIR, mask);				//1->output, 0->input
	IO_SET(gpio->PULLUP_EN, mask);			//set PULLUP_EN to disable pullup, clear pullup_en to enable pullup
	IO_CLR(gpio->PULLDOWN_EN, mask);		//set pulldown_en to enable pulldown, clear PULLDOWN_EN to disable pulldown
}
...
inline void pinAFIO(PIN_TypeDef pin, uint8_t opt) {
	...
	switch (opt) {
		case 1:
			//af mode 0
			IO_SET(gpio->AF_SEL, mask);			//1->af, 0->gpio
			IO_CLR(gpio->AF_S1 , mask);
			IO_CLR(gpio->AF_S0 , mask);
			break;
		case 2:
			//af mode 1
			IO_SET(gpio->AF_SEL, mask);			//1->af, 0->gpio
			IO_CLR(gpio->AF_S1 , mask);
			IO_SET(gpio->AF_S0 , mask);
			break;
		case 3:
			//af mode 2
			IO_SET(gpio->AF_SEL, mask);			//1->af, 0->gpio
			IO_SET(gpio->AF_S1 , mask);
			IO_CLR(gpio->AF_S0 , mask);
			break;
		case 4:
			//af mode 3
		...
		case 5:
			//gpio,
			IO_CLR(gpio->AF_SEL, mask);			//1->af, 0->gpio
			//IO_CLR(gpio->AF_S1 , mask);
			//IO_CLR(gpio->AF_S2 , mask);
			break;
		case 6:
			...
			IO_CLR(gpio->AF_SEL, mask);			//1->af, 0->gpio
			IO_CLR(gpio->DIR   , mask);			//1->output, 0->input
			IO_SET(gpio->PULLUP_EN , mask);		//1->disable pullup, 0->enable pullup
			IO_CLR(gpio->PULLDOWN_EN, mask);	//1->enable pulldown, 0->>disable pulldown
			break;
		case 7:
...
//generic print to uart
void uPrint(char *uRAM, int32_t dat) {
	uint8_t idx=20;
	char negFlag=0;							//1->negative, 0->positive
...
//default clock is rc
void RTCInit(void) {
	//pmuInit();							//initialize pmu - optional
#if defined(RTCOSC)
	RTCOSC();								//use user specified osc source
#else
	RTCOSC2RC();							//use rc oscillator as default
#endif
	RTCOSCCalib();							//start osc calibration
}

//initialize hardware rtc time
time_t time2RTC(time_t t) {
	...
	tmptr=gmtime(&t);
	RTCSetSec(DEC2BCD(tmptr->tm_sec));
	RTCSetMin(DEC2BCD(tmptr->tm_min));
	RTCSetHour(DEC2BCD(tmptr->tm_hour));
	RTCSetDay(DEC2BCD(tmptr->tm_mday));
	RTCSetMon(DEC2BCD(tmptr->tm_mon+1));				//month = 0..11
	RTCSetYear(DEC2BCD(tmptr->tm_year+1900-2000));		//RTC starts at 1/1/2000, posix year relative to 1/1/1900
	return t;
}
...
time_t RTC2time(time_t *t) {
	...
	tm.tm_mon=BCD2DEC(RTCGetMon())-1;					//month = 0..11
	tm.tm_year=BCD2DEC(RTCGetYear())+2000-1900;			//RTC starts at 1/1/2000, posix year relative to 1/1/1900
	if (t==NULL) return mktime(&tm);
	return *t=mktime (&tm);
}
//end pmu
