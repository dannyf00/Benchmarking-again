...
// v0.7f:02/17/2023 - updated rtc routines for ease of use
// v0.7g:02/23/2023 - added software rtc
// v0.7h:03/02/2023 - minor change to allow run-time updates to SystemCoreClock
//
//
//
//               |=======================|
...
#define CPU_CLK			CPU_CLK_8M			//cpu speed, 240M/160M/80M/40M/2M + 120M/24M/20M/12M/10M/8M/4M
#define uRAM_SIZE		32					//uxPrint buffer size, in bytes
#define RTCOSC()		RTCOSC2RC()			//RTCOSC2RC() or RTCOSC2Xtal(), user specified. Default is rtcOSCRC()

//configuration for circular buffer
/*******NOTE********************************/
/******cBUFF_SIZE: must be power of 2*******/
/*******************************************/
#define cBUFF_SIZE 		256					//must be power of 2: 0..32768 -> for buffers with more than 256 elements, change data type for writeIndex/readIndex
...
#define SysTick_PR		(1ul<<24)			//systick period
...
#define CPU_CLK_240M	2					//480/2 =240Mhz
#define CPU_CLK_120M	4					//480/4 =120Mhz
//>12: CLK_CLK_xx must be multiples of 4
#define CPU_CLK_60M		8					//480/8=48Mhz
#define CPU_CLK_40M		12					//480/12=40Mhz
#define CPU_CLK_30M		16					//480/16=30Mhz
#define CPU_CLK_24M		20					//480/20=24Mhz
#define CPU_CLK_20M		24					//480/24=20Mhz
#define CPU_CLK_16M		30					//480/30=16Mhz
#define CPU_CLK_12M		40					//480/40=12Mhz
#define CPU_CLK_10M		48					//480/48=10Mhz
#define CPU_CLK_8M		60					//480/60=8Mhz
#define CPU_CLK_6M		80					//480/80=6Mhz
#define CPU_CLK_4M		120					//480/120=4Mhz
#define CPU_CLK_2M		240					//480/240=2Mhz
//configure systemcoreclock
#define SystemCoreClock2MHZ()				SystemCoreClockConfig(CPU_CLK_2M)
#define SystemCoreClock4MHZ()				SystemCoreClockConfig(CPU_CLK_4M)
#define SystemCoreClock6MHZ()				SystemCoreClockConfig(CPU_CLK_6M)
#define SystemCoreClock8MHZ()				SystemCoreClockConfig(CPU_CLK_8M)
#define SystemCoreClock10MHZ()				SystemCoreClockConfig(CPU_CLK_10M)
#define SystemCoreClock12MHZ()				SystemCoreClockConfig(CPU_CLK_12M)
#define SystemCoreClock16MHZ()				SystemCoreClockConfig(CPU_CLK_16M)
#define SystemCoreClock20MHZ()				SystemCoreClockConfig(CPU_CLK_20M)
#define SystemCoreClock24MHZ()				SystemCoreClockConfig(CPU_CLK_24M)
#define SystemCoreClock30MHZ()				SystemCoreClockConfig(CPU_CLK_30M)
#define SystemCoreClock40MHZ()				SystemCoreClockConfig(CPU_CLK_40M)
#define SystemCoreClock60MHZ()				SystemCoreClockConfig(CPU_CLK_60M)
...
#define SystemCoreClock160MHZ()				SystemCoreClockConfig(CPU_CLK_160M)
...
void RTCInit(void);									//initialize rtc //default clock is rc
...
#define RTCGetSec()		((pmuRTC0() >> 0) & 0x3f)
#define RTCGetMin()		((pmuRTC0() >> 8) & 0x3f)	//datasheet is missing this item. see wm_regs.h
#define RTCGetHour()	((pmuRTC0() >>16) & 0x1f)
#define RTCGetDay()		((pmuRTC0() >>24) & 0x1f)
#define RTCGetMon()		((pmuRTC1() >> 0) & 0x0f)
#define RTCGetYear()	((pmuRTC1() >> 8) & 0x7f)
//clear rtc1's bit 16 before setting the time
//set rtc1's bit 16 to enable timing
#define RTCSetSec(x)	do {pmuRTC1() &=~(1<<16); (pmuRTC0() = (pmuRTC0() &~(0x3f << 0)) | (((x) & 0x3f) << 0)); pmuRTC1() |= (1<<16);} while (0)
#define RTCSetMin(x)	do {pmuRTC1() &=~(1<<16); (pmuRTC0() = (pmuRTC0() &~(0x3f << 8)) | (((x) & 0x3f) << 8)); pmuRTC1() |= (1<<16);} while (0)
#define RTCSetHour(x)	do {pmuRTC1() &=~(1<<16); (pmuRTC0() = (pmuRTC0() &~(0x1f <<16)) | (((x) & 0x1f) <<16)); pmuRTC1() |= (1<<16);} while (0)
#define RTCSetDay(x)	do {pmuRTC1() &=~(1<<16); (pmuRTC0() = (pmuRTC0() &~(0x1f <<24)) | (((x) & 0x1f) <<24)); pmuRTC1() |= (1<<16);} while (0)
#define RTCSetMon(x)	do {pmuRTC1() &=~(1<<16); (pmuRTC1() = (pmuRTC1() &~(0x0f << 0)) | (((x) & 0x0f) << 0)); pmuRTC1() |= (1<<16);} while (0)
#define RTCSetYear(x)	do {pmuRTC1() &=~(1<<16); (pmuRTC1() = (pmuRTC1() &~(0x7f << 8)) | (((x) & 0x7f) << 8)); pmuRTC1() |= (1<<16);} while (0)

time_t time2RTC(time_t t);					//initialize hardware rtc time
